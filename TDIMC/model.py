...
class TDIMC(torch.nn.Module):
    ...
    def forward(self, data, momentum, warm_up):
        ...
        if warm_up:
            # 在warm_up阶段，使用单位矩阵作为mask_pos，即假设每个样本与自身为正样本对（对角线为1）。
            mp = torch.eye(z[0].shape[0]).to(device)
            mp = [mp, mp]
        else:
            # 在非warm_up阶段，使用kernel_affinity计算mask_pos。
    ...
    def kernel_affinity(self, z, temperature=0.1, step: int = 5):
        ...
        alpha = 0.5
        G = torch.eye(G.shape[0]).to(device) * alpha + G * (1 - alpha)
        return G
    ...
    def extract_feature(self, data, mask):
        N = data[0].shape[0]
        z = [torch.zeros(N, self.feature_dim[i]).to(device) for i in range(self.n_views)]
        for i in range(self.n_views):
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, x_q, x_k, mask_pos=None):
        # mask_pos: shape [N, N], 1 for positive pair, 0 for negative pair
        x_q = L2norm(x_q)
        x_k = L2norm(x_k)
        N = x_q.shape[0]
        if mask_pos is None:
            mask_pos = torch.eye(N).to(device)
